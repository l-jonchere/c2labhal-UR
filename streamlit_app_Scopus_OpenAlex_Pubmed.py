...
def run(self):  # La fonction main() devient run()
        st.title("🥎 c2LabHAL")
        ...
        self.collection_a_chercher = st.text_input(
            "Collection HAL",
            value="",
            key="collection_hal_app1",
            help="Saisissez le nom de la collection HAL du laboratoire, par exemple MIP"
        )

        self.openalex_institution_id = st.text_input("Identifiant OpenAlex du labo", key="openalex_id_app1", help="Saisissez l'identifiant du labo dans OpenAlex, par exemple i4392021216")

        col1, col2 = st.columns(2)
        with col1:
            self.pubmed_id = st.text_input("Requête PubMed", key="pubmed_query_app1", help="Saisissez la requête Pubmed qui rassemble le mieux les publications du labo, par exemple ((MIP[Affiliation]) AND ((mans[Affiliation]) OR (nantes[Affiliation]))) OR (EA 4334[Affiliation]) OR (EA4334[Affiliation]) OR (UR 4334[Affiliation]) OR (UR4334[Affiliation]) OR (Movement Interactions Performance[Affiliation] OR (Motricité Interactions Performance[Affiliation]) OR (mouvement interactions performance[Affiliation])")
        with col2:
            self.pubmed_api_key = st.text_input("Clé API Pubmed", key="pubmed_api_key_app1", help="Pour obtenir une clé API, connectez vous sur Pubmed, cliquez sur Account, Account Settings, API Key Management.")

        col1, col2 = st.columns(2)
        with col1:
            self.scopus_lab_id = st.text_input("Identifiant Scopus du labo", key="scopus_lab_id_app1", help="Saisissez le Scopus Affiliation Identifier du laboratoire, par exemple 60105638")
        with col2:
            self.scopus_api_key = st.text_input("Clé API Scopus", key="scopus_api_key_app1", help="Pour obtenir une clé API : https://dev.elsevier.com/. Sinon, contactez la personne en charge de la bibliométrie dans votre établissement")

        col1, col2 = st.columns(2)
        with col1:
            self.start_year = st.number_input("Année de début", min_value=1900, max_value=2100, value=2020, key="start_year_app1")
        with col2:
            self.end_year = st.number_input("Année de fin", min_value=1900, max_value=2100, value=2025, key="end_year_app1")

        self.fetch_authors = st.checkbox("🧑‍🔬 Récupérer les auteurs sur Crossref", key="fetch_authors_app1")

        if self.fetch_authors:
            self.compare_authors = st.checkbox("🔍 Comparer les auteurs avec ma liste de chercheurs", key="compare_authors_app1")
            if self.compare_authors:
                self.uploaded_authors_file = st.file_uploader("📤 Téléversez un fichier CSV avec deux colonnes : 'collection', 'prénom nom'", type=["csv"], key="authors_file_app1")

        # Initialiser la barre de progression
        self.progress_bar = st.progress(0, key="progress_bar_app1")
        self.progress_text = st.empty()

        if st.button("Rechercher", key="rechercher_app1"):
